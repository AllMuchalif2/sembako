                                ...
                                @foreach ($categories as $category)
                                    <li>
                                        <a href="{{ route('products.index', ['category' => $category->id, 'search' => request('search')]) }}"
                                            class="block px-4 py-2 rounded-md text-sm font-medium {{ request('category') == $category->id ? 'bg-blue-100 text-blue-700' : 'text-gray-700 hover:bg-gray-200' }}">
                                            {{ $category->name }}
                                        </a>
                                    </li>
                                @endforeach
                        ...
                        <form action="{{ route('products.index') }}" method="GET" class="mb-8">
                            ...
                            <div class="relative">
                                ...
                                <input id="search" name="search"
                                    ...
                                    placeholder="Cari di kategori ini..." type="search" value="{{ request('search') }}">
                            </div>
                        </form>

                        <!-- Grid Produk -->
                        @if ($products->count())
                            <div class="grid grid-cols-2 gap-x-4 gap-y-10 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:gap-x-6">
                                @foreach ($products as $product)
                                    <div class="group relative">
                                        <div class="flex flex-col h-full bg-white rounded-lg shadow-md overflow-hidden transition-transform duration-300 ease-in-out group-hover:shadow-xl group-hover:-translate-y-1">
