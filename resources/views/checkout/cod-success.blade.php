        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-8 text-center">
                    ...
                    <p class="text-gray-600 mb-6">Pesanan Anda dengan metode pembayaran <span
                            class="font-semibold text-green-600">COD (Bayar di Tempat)</span> telah berhasil dibuat</p>
                    ...
                    <div class="bg-gray-50 rounded-lg p-6 mb-8 text-left">
                            ...
                            <ul class="space-y-2">
                                @foreach ($order->items as $item)
                                    <li class="flex justify-between text-sm">
                                        ...
                                        <span
                                            class="font-medium text-gray-900">Rp{{ number_format($item->subtotal, 0, ',', '.') }}</span>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span
                                    class="text-gray-900">Rp{{ number_format($order->total_amount - $order->shipping_cost + $order->discount_amount, 0, ',', '.') }}</span>
                            </div>
                            @if ($order->discount_amount > 0)
                                <div class="flex justify-between">
                                    <span class="text-gray-600">Diskon</span>
                                    <span
                                        class="text-green-600">-Rp{{ number_format($order->discount_amount, 0, ',', '.') }}</span>
                                </div>
                            @endif
                            <div class="flex justify-between">
                                <span class="text-gray-600">Ongkir</span>
                                <span
                                    class="text-gray-900">{{ $order->shipping_cost > 0 ? 'Rp' . number_format($order->shipping_cost, 0, ',', '.') : 'Gratis' }}</span>
                            </div>
                            <div class="flex justify-between border-t border-gray-300 pt-2 font-bold text-base">
                                <span class="text-gray-900">Total</span>
                                <span
                                    class="text-blue-600">Rp{{ number_format($order->total_amount, 0, ',', '.') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                ...
                                <ul class="list-disc list-inside text-sm text-blue-700 space-y-1">
                                    ...
                                    <li>Siapkan uang tunai sejumlah
                                        <strong>Rp{{ number_format($order->total_amount, 0, ',', '.') }}</strong> saat
                                        barang tiba</li>
            ...
            <div class="mt-6 bg-white rounded-lg shadow-sm p-6">
                ...
                <p class="text-sm text-gray-600 text-center">
                    Jika Anda memiliki pertanyaan tentang pesanan ini, silakan hubungi kami atau cek halaman transaksi
                    Anda untuk update status pesanan.
                </p>
            </div>
        </div>
    </div>
